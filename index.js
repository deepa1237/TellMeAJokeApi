...
app.get("/filter", (req, res) => {
  ...
  const response = jokes.filter((joke) => joke.jokeType == type );
  return res.status(200).send(response);
})
...
app.post("/jokes", (req, res) => {
  ...
  const response = jokes.push(newJoke);
...
app.put("/jokes/:id", (req, res) => {
  ...
  return res.status(200).json(jokes[searchIndex]);
})
